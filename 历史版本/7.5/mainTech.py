...
class Camera:
    ...
    def capture(self, corners=None):
        """
        corners=None:
            只做 lens_corr，返回原始视角图像。
        corners!=None:
            保留旧接口，必要时仍可 rotation_corr。
            但现在地图识别阶段不会再调用这个分支。
        """
        if corners is None:
...
class GameSymbol:
    ...
    def detect_grid_color_and_generate_map(self, img, map_corners, cell_inner_pad=1):
        """
        新地图识别逻辑：

        不再使用 rotation_corr。
        直接在当前原始图像 img 上，根据 detect_and_draw_wall_border 得到的外框矩形，
        把矩形内部切成 16x12 个格子，然后沿用原来的颜色分割识别逻辑。
        """
        ...
        # detect_and_draw_wall_border 返回的是：
        # [左上, 右上, 右下, 左下]
        ...
        for y_idx in range(GRID_Y_NUM):
            for x_idx in range(GRID_X_NUM):
                ...
                # 用左右边界算每个格子，避免 int(x_cell_size) 造成累计误差
    ...
    def get_perspective_corners(img, roi):
        """
        使用最小外接旋转矩形获取四个角点，并按 [左上, 右上, 右下, 左下] 排序。
        当前主流程没有使用它，保留备用。
        """
...
ELEMENT = {
    ...
    "box": {
        ...
        "symbol": "$"},
    "car_head": {
        "threshold": (65, 93, -51, -6, -49, 2),
        "symbol": None},
    "car_back": {
        "threshold": (63, 91, -84, -56, -1, 86),
        "symbol": None},
    "goal": {
...
while True:
    ...
    if not myuart.readFlag(b'O'):
        continue
    # 当前版本：detect_and_draw_wall_border 返回外框矩形四角
    map_corners = gamesymbol.detect_and_draw_wall_border(img, ROI)

    if map_corners is None:
        myuart.writeErr()
        continue
    # 小车检测继续使用 homography，不使用 rotation_corr
