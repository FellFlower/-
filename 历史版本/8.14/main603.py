...
# 小车动态ROI参数
# ==========================
# True：开启动态ROI。第一帧全图搜索，之后以上一帧小车中心为中心搜索局部ROI；
#       如果局部ROI识别失败，同一帧自动退回全图搜索。
# False：关闭动态ROI。每一帧都直接使用整张拉正图搜索小车。
...
class myUART:
    ...
    def readFlag(self, byteFlag):
        assert byteFlag in (b'O', b'P', b'S', b'B'), \
            "we only have b'O', b'P', b'S', b'B' 4 type flags"
...
class GameSymbol:
    ...
    def detect_output_car_coord(self,
        ...
        不拍第二张图，不做多帧加权。
        """
        # ENABLE_DYNAMIC_ROI是总开关：
        # 即使调用者传入use_tracking=True，只要总开关关闭，也强制使用全图搜索。
        if use_tracking and ENABLE_DYNAMIC_ROI:
            raw_coord = \
...
def send_current_coord_once():
    ...
    连续定位：
        单帧
        + 动态ROI/全图搜索（由ENABLE_DYNAMIC_ROI控制）
        + 不加权
        + 立即UART发送
    """
    if map_corners is None:
